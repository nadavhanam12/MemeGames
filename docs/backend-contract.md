...
### `POST /api/event` (not yet implemented server-side)

Fire-and-forget analytics event. The client (`Front/src/backend/analytics.ts`) already
POSTs to this path; the leaderboard server repo still needs to add the route and
storage for it — until then these requests just fail silently (best-effort, no
retry, never blocks gameplay).
...
```json
{
  ...
  "event": "run_end",
  "runId": "3fa2c1e0-....-....-....-............",
  "payload": { "score": 12345, "survivalTime": 210 },
  "clientTs": "2026-08-10T12:00:00.000Z"
}
```

Suggested response: `{ "accepted": true }`. No auth/rate-limit rules defined yet —
treat similarly to the abuse handling on `/api/token` if it gets scraped.

Events currently sent, one row per run except where noted:

- `run_start` — run begins.
- `day_end` (repeats per day) — `{ day, missionDone, safe, lost, price }`.
- `mission_result` (repeats per day) — `{ day, type, done }`.
- `upgrade_bought` (repeats per purchase) — `{ key, level }`.
- `run_end` — `{ score, survivalTime, daysSurvived, tankersSafe, tankersLost, bestCombo }`.
- `score_submitted` — `{ accepted, saved }`, sent when the submit-score form succeeds/fails.
- `score_submit_skipped` — sent when the player hits SKIP on the submit-score overlay instead.

Deliberately excluded to keep volume low: near-misses, combos, event-card
wins/losses, price-threshold crossings — revisit only if a specific balance
question needs them.
...
- `events` (not yet implemented): would store `game`, `event`, `runId`, `payload` (JSON), `clientTs`, `receivedAt`, requester IP.

## Known Bugs

- **Period-filtered `GET /api/leaderboard` returns empty `entries` despite a correct nonzero `total`.** Confirmed 2026-08-10 against the deployed server (`https://meme-games.t-war.co.il`): `period=daily` reported `total: 1` with `entries: []`; same for `weekly` (`total: 2`) and `monthly` (`total: 12`). Only `period=all` returns actual rows. Affects every client identically (not a per-browser/token issue) — likely a date-range `WHERE` bug in the period query against `current_high_scores`. Needs a fix in the leaderboard server repo (not this repo).
